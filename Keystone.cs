...
public sealed class Keystone<T>(T dbContext, string baseDir) where T : DbContext, IKeystoneDbContext
{
    ...
    private bool BaseDirectoryExists => Directory.Exists(baseDir);
    ...
    public IDisposable Start(CancellationToken cancellationToken = default)
    {
        ...
        keystoneProcess.SetConsoleOutError();
        _ = Task.Run(async () =>
        {
            try
            {
                return await keystoneProcess.ExecAsync("keystone", ["start"], cancellationToken);
            }
            catch
            {
                throw;
